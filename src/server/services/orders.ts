...
export function nextReference(merchantId: string): string {
  const row = get<{ c: number }>("SELECT COUNT(*) AS c FROM orders WHERE merchant_id = ?", [merchantId]);
  return `CMD-${String((row?.c ?? 0) + 1).padStart(5, "0")}`;
}

export function upsertCustomer(merchantId: string, name: string, phone: string, wilaya?: string | null, commune?: string | null, isTest = false) {
  const p = normalizeDzPhone(phone);
  const existing = get<{ id: string }>("SELECT id FROM customers WHERE merchant_id = ? AND normalized_phone = ?", [merchantId, p.normalized]);
  if (existing) {
    run("UPDATE customers SET full_name = COALESCE(NULLIF(?,''), full_name), wilaya = COALESCE(?, wilaya), commune = COALESCE(?, commune), updated_at = ? WHERE id = ?", [
      name,
      wilaya ?? null,
      commune ?? null,
      nowIso(),
...
export function recomputeCustomerStats(customerId: string) {
  const s = get<{ total: number; delivered: number; cancelled: number; returned: number; cod: number; last: string | null }>(
...
export function createOrder(input: OrderInput): { id: string; reference: string; duplicated: boolean } {
  const p = normalizeDzPhone(input.phone);
  const reference = input.reference?.trim() || nextReference(input.merchantId);

  // Idempotency for connector-sourced orders.
  if (input.externalId) {
    const dup = get<{ id: string; reference: string }>(
  ...
  const dupRef = get<{ id: string; reference: string }>("SELECT id, reference FROM orders WHERE merchant_id = ? AND reference = ?", [input.merchantId, reference]);
  ...
  tx(() => {
    const customerId = upsertCustomer(input.merchantId, input.customerName, input.phone, input.wilaya, input.commune, input.isTest);
    run(
    ...
    for (const item of input.items ?? []) {
      run("INSERT INTO order_items (id, merchant_id, order_id, product_name, variant, quantity, unit_price) VALUES (?,?,?,?,?,?,?)", [
        uid("itm"),
        input.merchantId,
        id,
        item.product_name,
    ...
    run("INSERT INTO order_events (id, merchant_id, order_id, type, title, description) VALUES (?,?,?,?,?,?)", [
      uid("evt"),
      input.merchantId,
      id,
      "status_change",
  ...
  bumpUsage(input.merchantId, "orders");
  onNewOrder(id);
...
export function listOrders(f: OrderFilters) {
  ...
  const total = get<{ c: number }>(`SELECT COUNT(*) AS c FROM orders o WHERE ${whereSql}`, params)?.c ?? 0;
...
export function orderDetail(merchantId: string, orderId: string) {
  ...
  return {
    order,
    items: all("SELECT * FROM order_items WHERE order_id = ? AND merchant_id = ?", [orderId, merchantId]),
    events: all("SELECT * FROM order_events WHERE order_id = ? AND merchant_id = ? ORDER BY created_at DESC LIMIT 100", [orderId, merchantId]),
    messages: all("SELECT * FROM whatsapp_messages WHERE order_id = ? AND merchant_id = ? ORDER BY created_at ASC LIMIT 200", [orderId, merchantId]),
    shipment: get("SELECT * FROM delivery_shipments WHERE order_id = ? AND merchant_id = ? ORDER BY created_at DESC LIMIT 1", [orderId, merchantId]),
    deliveryEvents: all("SELECT * FROM delivery_events WHERE order_id = ? AND merchant_id = ? ORDER BY occurred_at DESC LIMIT 50", [orderId, merchantId]),
    automationRuns: all("SELECT * FROM automation_runs WHERE order_id = ? AND merchant_id = ? ORDER BY created_at DESC LIMIT 50", [orderId, merchantId]),
  };
}
